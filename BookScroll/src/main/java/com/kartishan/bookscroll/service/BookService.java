...
public class BookService {
    ...
    public BookWithCategoriesDTO convertToDto(Book book) {
        ...
        BookWithCategoriesDTO bookDTO = BookWithCategoriesDTO.builder().id(book.getId()).name(book.getName()).author(book.getAuthor()).authorFull(book.getAuthorFull()).description(book.getDescription()).rating(book.getRating()).pageCount(book.getPageCount()).categories(categoryNames).build();
        return bookDTO;
    }

    public void addBookView(Book book) {
        BookView bookView = bookViewRepository.findByBookId(book.getId()).orElseGet(() -> {
    ...
    public void addBookViewHistory(User user, Book book) {
        UserBookViewHistory history = userBookViewHistoryRepository.findByUserAndBook(user, book)
                .orElseGet(() -> {
                    UserBookViewHistory newUserBookViewHistory = new UserBookViewHistory();
                    newUserBookViewHistory.setUser(user);
                    newUserBookViewHistory.setBook(book);
...
//    }
    ...
    public List<BookWithCategoriesAndHistoryDTO> getUserHistory(UUID userId) {
        try {
            List<UserBookViewHistory> histories = userBookViewHistoryRepository.findByUserId(userId);
            return histories.stream().map(this::convertToBookWithCategoriesAndHistoryDTO).collect(Collectors.toList());
        } catch (Exception e) {
            throw new RuntimeException("History not found! More: " + e);
        }
    }

    public BookWithCategoriesAndHistoryDTO convertToBookWithCategoriesAndHistoryDTO(UserBookViewHistory history) {
        BookWithCategoriesDTO bookDTO = convertToDto(history.getBook());
        return BookWithCategoriesAndHistoryDTO.builder()
                .id(bookDTO.getId())
                .name(bookDTO.getName())
                .author(bookDTO.getAuthor())
                .authorFull(bookDTO.getAuthorFull())
    ...
    public Book getBookByName(String name) {
        return bookRepository.findByName(name).orElseThrow(() -> new BookNotFoundException("Book with name " + name + " not found"));
    }
    ...
    public List<Book> getTopBooksByRatingAndViewCount(int limit) {
        ...
        Map<Book, Long> viewCounts = userBookViewHistoryRepository.findAll().stream().collect(Collectors.groupingBy(UserBookViewHistory::getBook, Collectors.counting()));
        ...
        return averageValues.entrySet().stream().sorted(Map.Entry.comparingByValue(Collections.reverseOrder())).map(Map.Entry::getKey).limit(limit).collect(Collectors.toList());
    }
